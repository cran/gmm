...
The paper is organized as follows. Section 2 presents the theoretical aspects of the GMM method along with several examples in economics and finance. Through these examples, the functionality of the \pkg{gmm} packages is presented in details. Section 3 presents the GEL method with some of the examples used in section 2. Section 4 concludes.
...
E[g(\theta_0,x_i)] = 0 ,
\end{equation}
where $x_i$ is a vector of cross-sectional data, time series or both. In order for GMM to produce consistent estimates from the above conditions, $\theta_0$ has to be the unique solution to $E[g(\theta,x_i)]=0$ and be an element of a compact space. Some boundness assumptions on higher moments of $g(\theta,x_i)$ are also required. However, it does not impose any condition on the distribution of $x_i$, except for the degree of dependence of the observations when it is a vector of time series. 
...
library(gmm)
@
In order to reproduce the following results, it is important not to use a version prior to 1.1-0. Major changes have been made to harmonize results and methods to the \code{lm} procedure. Because of these similarities, functions such as \code{linear.hypothesis} from the \pkg{car} package can be applied to \code{gmm} or \code{gel} objects as it is applied to \code{lm} objects.   
...
@
First we generate normally distributed random numbers and compute the ML estimates of the two parameters:
<<>>=
set.seed(123)
n <- 200
x1 <- rnorm(n, mean = 4, sd = 2)
print(mu <- mean(x1))
print(sig <- sqrt(var(x1)*(n-1)/n))
@
...
summary(res)
@
A small simulation using the following function shows that ML is more efficient than GMM in that case.
<<>>=
...
GMM& -3.991121e-05 &0.09270708 &  0.09266073&  -8.156671e-02 & 0.04650049& 0.05313037\\ 
ML& 0.002113659  &0.0822847  &  0.08224803 & -0.034948166 & 0.0410531& 0.04225395 \\ \hline
\end{tabular}
\end{center}

\subsubsection{Estimating the parameters of a stable distribution}
...
\]
where $\Psi(\theta;\tau_l)$ is the characteristic function. There are more than one way to define a stable distribution and it depends on the choice of parametrization. We will follow the notation of \cite{nolan09} and consider stable distributions $S(\alpha,\beta,\gamma,\delta;1)$, where $\alpha\in (0,2]$ is the characteristic exponent and $\beta\in[-1,1]$, $\gamma>0$ and $\delta\in\R$ are respectively the skewness, the scale and the location parameters. The 1 defines the parametrization. The \pkg{fBasics} package offers a function to generate random variables from stable distributions and uses the same notation. This parametrization implies that:
\[
\Psi(\theta;\tau_l) = 
\left\{
\begin{array}{lcr}
...
summary(res)
@
The null hypothesis that the moment conditions are satisfied is rejected. It seems that \code{nlminb} is more reliable then \code{optim} when there are restrictions on the parameter space. The option \code{optfct} can be modified to use \code{nlminb} instead. In that case, we can specify the upper and lower bounds of $\theta$. 
<<>>= 
res2 <- gmm(g2,x2,t0,optfct="nlminb",lower=c(0,-1,0,-Inf),upper=c(2,1,Inf,Inf))
summary(res2)
@
We conclude this example by estimating the parameters for a vector of stock returns from the data set \code{Finance} that comes with the package.
<<>>=
data(Finance)
...
\subsubsection{A linear model with iid moment conditions}
...
y <- 0.1*w + e[,2]
@ 
where \code{rmvnorm} is a multivariate normal distribution random generator which is included in the package \pkg{mvtnorm}. For a linear model, the $g$ argument is a formula that specifies the right- and left-hand sides as for \code{lm} and $x$ is the matrix of instruments:
<<>>=
...
The moment conditions of this example are iid. Therefore, we can add the option \code{vcov="iid"}. This option tells \code{gmm} to estimate  the covariance matrix of $\sqrt{n}\bar{g}(\theta^*)$ as follows:
\[
\hat{\Omega}(\theta^*) = \frac{1}{n}\sum_{i=1}^n g(\theta^*,x_i)g(\theta^*,x_i)'
\]
However, it is recommended not to set this option to "iid" in practice. Finally, it is not necessary to provide the gradient when the model is linear since it is already included in \code{gmm}. The first results are:
<<>>=
...
<<>>=
res2 <- gmm(g3,x=h,type='iterative')
summary(res2)$coef
@
where the  option \code{crit=} (default is $10^{-7}$) can be set as small as we want to increase the precision and \code{maxiter=} can be increased if ITGMM fails to converge. 
...
summary(res3)$coef
@
It is possible to produce confidence intervals by using the method \code{confint} the same way it is applied to \code{lm} objects:
<<>>=
...
lines(w,fitted(lm(y~w)),col=3)
legend("topleft",c("Data","Fitted GMM","Fitted LS"),col=1:3,lty=c(1,1,1))
@

The method \code{abline} would have produced the same result. In fact, \code{lines(w,fitted(res))} is equivalent to \code{abline(res)}. The LS seems to better fit the model. But the graphics hides the endogeneity problem. LS overestimates the relationship between $y$ and $w$ because it does not take into account the fact that some of the correlation is caused by the fact that $y_i$ and $w_i$ are positively correlated with the error term $\epsilon_i$.  

Finally, the \code{plot} method produces some graphics to analyze the properties of the residuals. It can only be applied to \code{gmm} objects when \code{g} is a formula.
...
xt1 <- x5[6:(t-1)]
xt2 <- x5[5:(t-2)]
h2 <- cbind(x5[4:(t-3)],x5[3:(t-4)],x5[2:(t-5)],x5[1:(t-6)])
g4 <- xt~xt1+xt2
res <- gmm(g4,x=h2)
summary(res)
@
...
<<>>=
res2 <- gmm(g4,x=h2,kernel="Truncated")
coef(res2)
res3 <- gmm(g4,x=h2,kernel="Bartlett")
coef(res3)
res4 <- gmm(g4,x=h2,kernel="Parzen")
coef(res4)
res5<- gmm(g4,x=h2,kernel="Tukey-Hanning")
coef(res5)
@
The similarity of the results is not surprising since the matrix of weigths should only affect the efficiency of the estimator.  We can compare the estimated standard deviations using the method \code{vcov}:
<<>>=
diag(vcov(res2))^.5
diag(vcov(res3))^.5
diag(vcov(res4))^.5
...
\subsubsection{Estimating a system of equations: CAPM}
...
@
Another way to test the CAPM is to estimate the restricted model ($\alpha=0$), which is over-identified,  and to perform a J-test. Adding $-1$ to the formula removes the intercept. In that case, a column of ones has to be added to the matrix of intruments:
<<>>=
...
\subsubsection{Testing the CAPM using the stochastic discount factor representation}
...
\subsubsection{Estimating continuous time processes using discrete time approximation}
...
\subsubsection{Comments on models with panel data}
...
res <- gmm(y~x,h)
@
The package \pkg{plm} offers several functions to manipulate panel data. It could therefore be combined with \pkg{gmm} when estimating such models. It also offers a way to estimate them with its own GMM algorithm for panel data. 
...
\end{equation}
Equation (\ref{gel_obj}) represents a saddle point problem. The solution is obtained by solving simultaneously two optimization problems. We can solve for $\theta$ by minimizing $P_n(\theta,\lambda(\theta))$, where $\lambda(\theta)$ is the solution to $\arg\max_\lambda P_n(\theta,\lambda)$ for a given $\theta$. Therefore an optimization algorithm needs to be called inside the $P_n(theta,\lambda)$ function. It makes the GEL very hard to implement numerically. For example, \cite{guggenberger08}, who analyzes the small sample properties of GEL, uses an home made Newton algorithm for $\lambda$ and a grid search for $\theta$ in order to confidently reach the absolute minimum. Using an home made algorithm for $\lambda$ makes the problem less  computationally demanding and does not seem to affect the properties of the estimator of $\theta_0$ as it was shown by \cite{guggenberger-hahn05}. 
...
\]
starting with $\lambda=0$, which corresponds to its asymptotic value. The algorithm stops when $\|\lambda_l-\lambda_{l-1}\|$ reaches a certain tolerance level. The second option is to let \code{optim} solves the problem. Then, as for \code{gmm}, the minimization problem is solved either by \code{optim}, \code{nlminb} or \code{constrOptim}.
...
For this example, we can leave the option \code{smooth} to its default value, which is \code{FALSE}, because of the iid properties of $x$.  
...
summary(res_cue)$coef
@
A fourth method is available which is called the exponentially tilted empirical likelihood (ETEL) and was proposed by \cite{schennach07}. However, it does not belong to the family of GEL estimators. It solves the problem of misspecified models. In such models there may not exist any pseudo value to which $\hat{\theta}$ converges as the sample size increases. ETEL uses the $\rho()$ of ET to solve for $\lambda$ and the $\rho()$ of EL to solve for $\theta$. She shows that ETEL shares the same asymptotic properties of EL without having to impose restrictions on the domain of $\rho(v)$ when solving for $\lambda$. 
<<>>=
...
Because the moment conditions are weakly dependent, we need to set the option \code{smooth=TRUE} and to select the kernel. Before going to the estimation procedure, we need to understand the relationship between the smoothing kernel and the HAC estimator that will result from this choice. The reason why we need to smooth the moment function is that GEL estimates the covariance matrix of $\bar{g}(\theta,x_t)$, as if we had iid observations, using the expression $(1/T)\sum_{t=1}^T(g_tg_t')$. We can show that substituting $g_t$ by $g^w_t$ in this expression results in an HAC estimator. However, the relationship between the smoothing kernel and the kernel that appears in the HAC estimator is not obvious. For example, we can show that if the smoothing kernel is Truncated, then the kernel in the HAC estimator is the Bartlett. Let us consider the truncated kernel with a bandwidth of 2. This implies that $w(s)=1/5$ for $|s|\leq 2$ and 0 otherwise. Then, the expression for the covariance matrix becomes:
\begin{eqnarray*}
...
<<>>=
tet0 <- gmm(g4,x=h2,wmatrix="ident")$coef
res <- gel(g4,x=h2,tet0,smooth=TRUE,kernel="Truncated")
summary(res)
@
The plot method produces one more graphics when applied to a \code{gel} object. It shows the implied probabilities along with the empirical density $(1/T)$. It allows to see which observations have more influence:
...
<<eval=false>>=
res <- gel(g4,x=h2,tet0,smooth=TRUE,kernel="Truncated",constraint=TRUE, ui=ui,ci=ci)
@
...
<<eval=false>>=
res <- gel(g4,x=h2,tet0,smooth=TRUE,kernel="Truncated",optlam="numeric")
@
which produce in this case almost identical results. However, it takes much more time than using the iterative method. 

\subsubsection{Comments}

The GEL method is very unstable numerically. This fact has been reported many times in the recent literature. The method has been included in the \pkg{gmm} package because recent theoretical evidences suggest that it may produce better estimators than GMM. Because \proglang{R} is an open source statistical package, it offers a good platform to experiment its numerical properties and to improve the algorithm used to obtain the results.  


\section{Conclusion}

The \pkg{gmm} package offers complete and flexible algorithms to estimate models by GMM and GEL. Several options are available which allow to choose among several GMM and GEL methods and many different HAC matrix estimators. In order to estimate the vector of parameters, users can select their preferred optimization algorithm depending on whether inequality constraints are required. For the vector of Lagrange multiplier of GEL, it can be computed by an home made Newton method which increases the speed of convergence and reduce the instability of the estimation procedure. It could then easily be used by those who are interested in studying the numerical properties of both methods.   
